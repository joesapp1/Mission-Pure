...
$SHORT_CACHE = "no-cache, must-revalidate, max-age=0"

Write-Host "Deploying Mission Pure to Google Cloud..." -ForegroundColor Cyan

# HTML pages - no cache (must revalidate on every visit)
...
# Flush entire CDN cache (synchronous — waits for completion)
Write-Host "  Clearing CDN cache..." -ForegroundColor Yellow
gcloud compute url-maps invalidate-cdn-cache $LB --path="/*" --project=$PROJECT 2>&1 | Out-Null
