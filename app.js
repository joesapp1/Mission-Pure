const APP_BUILD = "20260624192904";

const STORAGE_KEY = `missionPure.waterLookup:${APP_BUILD}`;

const DATASET_URL = "data/water-data.json?v=20260624192904";
const DFW_ZIP_MAP_URL = "data/zip-to-utilities-dfw.json?v=20260624192904";
const CHEM_INFO_URL = "data/chemical-info.json?v=20260624192904";
...
const NOVA_AVATAR_SRC = "assets/BotIcon.webp?v=20260517-1353";
...
const NOVA_MIN_MATCH_SCORE = 0.78;
...
function novaMaybeAutogreet() {
  ...
  pushNovaMessage("Nova", `${timeGreet}! I'm Nova — Mission Pure's water guide. I can look up what's actually in your tap water by ZIP, compare Puronics systems, explain health risks, or get you booked with our install team. What brings you here today?`);
}
...
function novaPickFallback() {
  const opts = [
    "Hmm, let me try a different angle — I cover ZIP water lookups, contaminant health impacts, Puronics system comparisons, pricing, install timelines, and financing. Which one fits your question?",
    "Not quite sure I caught that one. Try asking about your ZIP code, a specific contaminant, which filter system fits your home, or how to book an install — or pick a quick button below!",
    "I want to give you the right answer, not a guess. Ask me about water quality in your area, Puronics products, health risks, scheduling, or pricing and I'll be right on it.",
  ];
  return opts[Math.floor(Math.random() * opts.length)];
}

function showNovaTyping() {
  if (!els.novaMessages) return null;
...
function hideNovaTyping(el) {
  if (el && el.parentNode) el.parentNode.removeChild(el);
}

async function novaDelayedReply(text, options = {}) {
  const typingEl = showNovaTyping();
  const delay = Math.min(500 + Math.min(text.length, 200) * 2, 1500);
  await new Promise((resolve) => setTimeout(resolve, delay));
  hideNovaTyping(typingEl);
  pushNovaMessage("Nova", text, options);
}

async function submitNovaText(text) {
  ...
  if (zipMatch && !/\d{6,}/.test(text)) {
    novaMissCount = 0;
    await novaDelayedReply(`Got it — pulling up the ZIP lookup for ${zipMatch[1]} so you can see local utilities and contaminants.`);
  ...
  if (entry && score >= NOVA_MIN_MATCH_SCORE) {
    ...
    await novaDelayedReply(reply, { allowHtml: entry.allowHtml });
    if (entry.action) {
  ...
  if (novaMissCount >= NOVA_MAX_MISSES) {
    await novaDelayedReply(
      `Let me get the right person on this — our team is available 8am–8pm daily. Call us at ${NOVA_CALL_TEXT} or tap below. <a class="nova-call-btn" href="${NOVA_CALL_TEL}">Call Mission Pure</a>`,
      { allowHtml: true }
    );
    novaMissCount = 0;
    return;
  ...
  // Near miss: plausible candidate — offer it as a clarifying response.
  if (entry && score >= NOVA_CLARIFY_SCORE) {
    const reply = typeof entry.response === "function" ? entry.response(text) : entry.response;
    await novaDelayedReply("I think this covers what you're asking — let me know if I'm off base:");
    pushNovaMessage("Nova", reply, { allowHtml: entry.allowHtml });
    renderNovaSuggestions(NOVA_SUGGESTION_PRESETS);
    return;
  }

  await novaDelayedReply(novaPickFallback());
  renderNovaSuggestions(NOVA_SUGGESTION_PRESETS);
}
...
(function seedNovaKnowledgeBase() {
  ...
  // ── Puronics product-specific intents ──────────────────────────────────
  registerNovaDialogue(
    ["terminator", "hydronex", "clarius", "filtramax", "defender", "duofresh", "micromax", "puronics system", "puronics product"],
    `Mission Pure carries the full Puronics residential lineup — chlorostatic (Terminator® iGen® C, Hydronex® iGen® C), bacteriostatic (iGen® B series), salt-free conditioning (Clarius™ iGen®), specialty problem-water systems, and the Micromax™ 8500 five-stage RO. <a href="products.html">Browse all Puronics systems →</a>`,
    { allowHtml: true }
  );

  registerNovaDialogue(
    ["what products do you carry", "what systems do you sell", "which puronics", "puronics lineup", "product list", "see products", "products page"],
    `We carry the full Puronics residential lineup — whole-home softeners, chlorostatic systems, salt-free conditioners, bacteriostatic systems, and under-sink RO. <a href="products.html">See all systems on our Products page →</a>`,
    { allowHtml: true }
  );

  registerNovaDialogue(
    ["micromax 8500", "ro system", "5 stage ro", "five stage ro", "drinking water system", "reverse osmosis system"],
    `The Puronics Micromax™ 8500 is Mission Pure's flagship under-sink RO — five stages including a high-rejection membrane that strips PFAS, lead, chromium-6, nitrates, and dissolved solids to near-zero at the kitchen tap. It feeds the fridge, ice maker, and a dedicated faucet in one install. <a href="products.html#ro">See RO options →</a>`,
    { allowHtml: true }
  );

  registerNovaDialogue(
    ["salt free", "no salt", "salt-free", "clarius", "template assisted", "tac system"],
    `The Puronics Clarius™ iGen® series conditions hardness minerals without salt or drain lines — minerals stay in the water but can't form scale on fixtures, appliances, or pipes. Great for homeowners who want soft-water results without the ongoing salt cost. <a href="products.html#salt-free">See Clarius systems →</a>`,
    { allowHtml: true }
  );

  registerNovaDialogue(
    ["chlorostatic", "chlorine system", "chloramine system", "water softener", "whole home system", "whole-home system"],
    `For chlorine/chloramine, Mission Pure installs Puronics Terminator® iGen® C or Hydronex® iGen® C — chlorostatic whole-home systems with catalytic carbon tuned for DFW municipal water. Want me to help you pick based on your home size and ZIP?`,
    {}
  );

  registerNovaDialogue(
    ["bacteriostatic", "bacteria system", "silverShield", "silver shield"],
    `Puronics bacteriostatic systems (Terminator® iGen® B, Hydronex® iGen® B) use SilverShield® HYgene® technology to inhibit bacterial growth inside the tank — ideal for homes on well water, boil-notice-prone areas, or anyone who wants that extra layer of safety.`,
    {}
  );

  registerNovaDialogue(
    ["what makes you different", "why mission pure", "why choose you", "competitor", "versus other companies", "compare to others"],
    `A few things set us apart: (1) We're a factory-authorized Puronics dealer — not a reseller, so you get manufacturer warranties and support. (2) We use real ZIP-based contaminant data to match the system to YOUR water. (3) Licensed installers, same-day installs, and we handle permits. (4) We're local to DFW and available 7 days a week. Want to see the full Puronics lineup? <a href="products.html">Browse products →</a>`,
    { allowHtml: true }
  );

  registerNovaDialogue(
    ["how much does it cost", "price range", "how much is it", "average cost", "total cost", "what does it cost"],
    `Whole-home Puronics systems start around the mid-$3,000s installed — that covers the tank, media, bypass manifold, labor, and warranty registration. Under-sink RO (Micromax™ 8500) is a fraction of that. Financing with 0% promo periods is available for qualified buyers. Want an exact quote for your home?`,
    {}
  );
...
const els = {
  ...
  hotSearchPreviewSection: document.getElementById("hotSearchPreview"),
  hotSearchPreviewStatus: document.getElementById("hotSearchPreviewStatus"),
  hotSearchPreviewCard: document.getElementById("hotSearchPreviewCard"),
  hotSearchPreviewQuery: document.getElementById("hotSearchPreviewQuery"),
  hotSearchPreviewTitle: document.getElementById("hotSearchPreviewTitle"),
  hotSearchPreviewIntro: document.getElementById("hotSearchPreviewIntro"),
  hotSearchPreviewBullets: document.getElementById("hotSearchPreviewBullets"),
  hotSearchPreviewMeta: document.getElementById("hotSearchPreviewMeta"),
  hotSearchPreviewCta: document.getElementById("hotSearchPreviewCta"),
};
...
async function loadHotSearchPreview() {
  const statusEl = els.hotSearchPreviewStatus;
  const card = els.hotSearchPreviewCard;
  const queryEl = els.hotSearchPreviewQuery;
  const titleEl = els.hotSearchPreviewTitle;
  const introEl = els.hotSearchPreviewIntro;
  const bulletsEl = els.hotSearchPreviewBullets;
  const metaEl = els.hotSearchPreviewMeta;
  const ctaEl = els.hotSearchPreviewCta;

  if (!statusEl || !card || !queryEl || !titleEl || !introEl || !metaEl || !ctaEl) {
    return;
  }

  const setStatus = (message, hideCard = true) => {
    statusEl.textContent = message;
    if (hideCard) card.hidden = true;
  };

  try {
    const divider = HOT_SEARCH_FEED_URL.includes("?") ? "&" : "?";
    const response = await fetch(`${HOT_SEARCH_FEED_URL}${divider}t=${Date.now()}`, { cache: "no-store" });
    if (!response.ok) {
    ...
    if (!post) {
      setStatus("No trending searches yet. Check back later.");
      return;
    }

    const generated = payload.generatedAt ? new Date(payload.generatedAt) : null;
    if (generated && !Number.isNaN(generated.valueOf())) {
      statusEl.textContent = `Generated ${generated.toLocaleString("en-US", { timeZone: "America/Chicago" })}`;
    } else {
      statusEl.textContent = "Latest Search Console trend";
    }

    queryEl.textContent = `Trending search: “${post.query}”`;
    titleEl.textContent = post.title || post.query;
    introEl.textContent = post.intro || "Mission Pure watches this topic so you can act immediately.";

    if (bulletsEl) {
      bulletsEl.innerHTML = "";
      (post.bullets || []).slice(0, 3).forEach((text) => {
        const li = document.createElement("li");
        li.textContent = text;
        bulletsEl.appendChild(li);
      });
      bulletsEl.hidden = bulletsEl.children.length === 0;
    }

    const metaBits = [];
    if (post.region) metaBits.push(`Region: ${post.region}`);
    if (post.topicId) metaBits.push(`Topic: ${post.topicId}`);
    metaEl.textContent = metaBits.join(" · ") || "North Texas trend";

    const ctaUrl = resolveHotSearchCta(post.ctaUrl);
    ctaEl.href = ctaUrl;
    ctaEl.textContent = post.ctaLabel || "See every hot search";

    card.hidden = false;
  } catch (error) {
    console.error("Failed to load hot search preview", error);
    setStatus("Couldn't load the latest hot search.");
  }
}

function resolveHotSearchCta(url) {
  if (!url) return "hot-searches.html";
  if (/^https?:/i.test(url)) {
    return url;
  }
  return url.replace(/^\.\/?/, "");
